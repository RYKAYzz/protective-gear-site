    ...
    .map((p) => {
      ...
      <article class="row" data-slug="${esc(p.slug)}" data-open="${open}" data-dirty="${dirty.has(p.slug)}">
        <button class="row__head" data-act="toggle">
          ...
          <span class="row__price">${p.price ? "From KSh " + Number(p.price).toLocaleString() + (p.unit ? " / " + esc(p.unit) : "") : ""}</span>
...
function body(p) {
  ...
  <div class="row__body">
    ...
    <div class="grid2">
      <label class="field"><span>Price — from (KSh), blank to hide</span>
        <input data-f="price" type="number" min="0" value="${p.price ?? ""}" /></label>
      <label class="field"><span>Price is per…</span>
        <select data-f="unit">
          ${["", "piece", "pair", "box", "pack", "carton", "roll", "set", "litre", "unit"]
            .map(
              (u) =>
                `<option value="${u}"${(p.unit || "") === u ? " selected" : ""}>${u ? u.charAt(0).toUpperCase() + u.slice(1) : "— not set —"}</option>`
            )
            .join("")}
        </select></label>
    </div>
